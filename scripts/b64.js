 ...
 */
export const decode = (s) => binaryToAscii(base64ToBinary(s));

/**
 * Encode ASCII string to base64 string
 */
export const encode = (s) => binaryToBase64(asciiToBinary(s));

/**
 * Convert a string to a Uint8Array
 * FIXME: TextEncoding and TextDecoding are not actually inverses
 */
export const asciiToBinary = (text) => _enc.encode(text);

/**
 * Convert a Uint8Array to a string
 * FIXME: TextEncoding and TextDecoding are not actually inverses
 */
export const binaryToAscii = (binary) => _dec.decode(binary);

/**
 * Return a base64-encoded string from a Uint8Array input
 */
export const binaryToBase64 = (originalBytes) => {
  // Pad the output array to a multiple of 3 bytes
  const length = originalBytes.length;
  const added = length % 3 === 0 ? 0 : 3 - (length % 3);
  const bytes = new Uint8Array(length + added);
